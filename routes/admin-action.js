...
function destroy_sessions_pred(res, predicate) {
  dbPool.query(
    ...
    function (err, rows, fields) {
      if (err) {
        logger.error(
          `DB error on SELECT while destroying session(s): ${err.code}: ${err.sqlMessage}`
        );
        sql_err(res, err);
        return;
      }
      ...
      if (ids.length > 0) {
        const ids_qms =
          'session_id = ' + ids.map(i => '?').join(' OR session_id = ');
        dbPool.query(
          'DELETE FROM sessions WHERE ' + ids_qms,
          ids,
          function (err, rows, fields) {
            if (err) {
              logger.error(
                `DB error on DELETE while destroying session(s): ${err.code}: ${err.sqlMessage}`
              );
              sql_err(res, err);
              return;
            }
  ...
  function (req, res, next) {
    if (
      chk_validation_errs(
        req,
        res,
        'invalid username, full name, class, balance, or password'
      )
    )
      return;
    ...
    bcrypt.hash(req.body.password, 10, function (err, hash) {
      ...
      dbPool.query(query, function (err, rows, fields) {
        if (err) {
          logger.error(
            `DB error while adding a user: ${err.code}: ${err.sqlMessage}`
          );
          sql_err(res, err);
        } else {
  ...
  function (req, res, next) {
    ...
    parseCSV(req.body.table, function (err, rows) {
      ...
      rows = rows.map(r => {
        ...
        r[3] = r[3] === '' ? null : r[3];
        for (var i = 5; i <= 8; i++) r[i] = vr.toBoolean(r[i], true);
        return r;
      });

      const query =
        'INSERT INTO users (username, password, full_name, klase, balance, can_send, can_receive, is_station, is_frozen) VALUES ' +
      ...
      dbPool.query(query, values, function (err, rows, fields) {
        if (err) {
          logger.error(
            `DB error while adding users in bulk: ${err.code}: ${err.sqlMessage}`
          );
          sql_err(res, err);
          return;
        }
    ...
    Q.ninvoke(dbPool, 'getConnection')
      .then(function (conn) {
        ...
        return Q.ninvoke(
          dbConn,
          'query',
          'SELECT id, id_from, id_to, amount FROM transactions WHERE id = ?',
          req.body.id
        );
      })
      .then(function (rows) {
      ...
      .then(function () {
        return Q.ninvoke(
          dbConn,
          'query',
          'UPDATE transactions SET reverted = ? WHERE id = ? LIMIT 1',
          [r, t.id]
        );
      })
      .then(function () {
      ...
      .catch(function (err) {
        dbConn.rollback(function () {
          dbConn.release();
          logger.error(
            `DB error while ${un}reverting transaction #${req.body.id}: ${err.code}: ${err.sqlMessage}`
          );
          sql_err(res, err);
        });
      })
...
router.post(
  ...
  function (req, res, next) {
    if (chk_validation_errs(req, res, 'invalid id or password')) {
      logger.warn(
        'invalid id or password passed to /admin-action/change-password'
      );
      return;
    }
    ...
    bcrypt.hash(req.body.password, 10, function (err, hash) {
      if (err) {
        logger.error(
          `bcrypt error while changing the password of ${req.body.id}`
        );
        next(err);
        return;
      }
      dbPool.query(
        ...
        function (err, rows, fields) {
          if (err) {
            logger.error(
              `DB error while changing the password of ${req.body.id}: ${err.code}: ${err.sqlMessage}`
            );
            sql_err(res, err);
            return;
          }
...
router.post(
  ...
  function (req, res, next) {
    ...
    dbPool.query(
      'UPDATE users SET is_frozen = ? WHERE id = ? LIMIT 1',
      [req.body.fr, req.body.id],
      function (err, rows, fields) {
        if (err) {
          logger.error(
            `DB error while ${un}freezing ${req.body.id}: ${err.code}: ${err.sqlMessage}`
          );
          sql_err(res, err);
