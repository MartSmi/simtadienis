  ...
  var rowId;
	...
	Q.ninvoke(dbPool, 'getConnection')
		...
		.then(function(rows) {
			...
			if (req.body.amount > balance) {
				...
				return Q.ninvoke(dbConn, 'query', 'SELECT * FROM auction ORDER BY id DESC LIMIT 1', []);
    ...
    .then(function (rows) {
      if (rows === 'failure') return 'failure';
				...
				logger.info(`${from_full_name} attempted to bet but auction has not started`);
				res.json({
					success: false,
      ...
      if (req.body.amount <= biggestBet) {
				logger.info(`${from_full_name} attempted to bet less than biggest bet (${req.body.amount} < ${biggestBet})`);
				res.json({
					success: false,
        ...
        logger.info(`${from_full_name} attempted to bet when not in progress (auction)`);
				res.json({
					success: false,
      ...
      //return Q.ninvoke(dbConn, 'beginBet');
		...
		.then(function(ret) {
			...
			return Q.ninvoke(
				dbConn, 'query',
				'UPDATE auction SET biggest_bet = ? WHERE id = ?', [req.body.amount, rowId]
			)
		})
    .then(function(ret) {
			if (ret === 'failure') return 'failure';
			return Q.ninvoke(
				dbConn, 'query',
				'UPDATE auction SET bettor_id = ? WHERE id = ?', [req.body.fromID, rowId]
			)
		})
		.then(function(rows) {
			if (rows === 'failure') return 'failure';
		...
		.then(function(ret) {
			...
			logger.log('info', `bet in auction: [${req.body.amount}] ${from_full_name}`);
