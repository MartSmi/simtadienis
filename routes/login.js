...
router.post('/', [
	...
	var query = 'SELECT id, full_name, klase, password, is_station FROM users WHERE username = ?';
	dbPool.query(query, req.body.username, function(error, rows, fields) {

		if (error || rows.length > 1) {
			logger.error(error ? `DB error on login: ${error}` : '***NON-UNIQUE USERNAME***');
		...
		bcrypt.compare(req.body.password, rows[0].password, function(err, valid) {
			if (err) {
				...
				logger.info(`failed login (bad pw) on ${rows[0].full_name} (${rows[0].klase})`);
				res.redirect(303, req.baseUrl + '?badcreds=true');
			}
			else {
