...
router.post(
  ...
  function (req, res, next) {
    ...
    if (req.session.guest) {
      logger.warn(`a guest tried to transfer funds (userID: ${req.ip})`);
      res.json({
        success: false,
        error: 'Svečiai negali perversti pinigų',
      });
      return;
    }
    ...
    Q.ninvoke(dbPool, 'getConnection')
      ...
      .then(function (rows) {
        ...
        if (req.body.amount > balance) {
          ...
          return Q.ninvoke(
            dbConn,
            'query',
            'SELECT id, full_name, balance, can_receive, is_station, is_frozen FROM users WHERE username = ?',
            req.body.to_account
          );
        }
      })
      .then(function (rows) {
        if (rows === 'failure') return 'failure';
          ...
          logger.info(
            `${from_full_name} attempted a transfer to non-existant account`
          );
          res.json({
            success: false,
        ...
        if (toID === fromID) {
          ...
          logger.info(
            `${from_full_name} attempted a transfer to a non-can_send account (${row.full_name})`
          );
          res.json({
            success: false,
          ...
          logger.info(
            `${from_full_name} attempted a transfer to a frozen account (${row.full_name})`
          );
          res.json({
            success: false,
          ...
          logger.warn(
            `the station ${from_full_name} attempted a transfer to another station (${row.full_name})`
          );
          res.json({
            success: false,
      ...
      .then(function (ret) {
        ...
        logger.log(
          high ? 'warn' : 'info',
          `transfer: [${req.body.amount}] ${from_full_name} -> ${to_full_name}`
        );
